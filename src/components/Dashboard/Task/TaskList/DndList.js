...
const DNDList = props => {
	const { completeTask, getExpandedTreeArr, getCompletedTasks, taskState, getDragState, setDragState } = useTreeDataUtils();
	...
	return (
		<>
			<div
				...
				style={{
					height: getExpandedTreeArr(constants.DRAG_FROM.MAIN, 'incomplete').length * constants.ITEM_HEIGHT,
					transition: "height 0.3s cubic-bezier(0, 0.86, 0.61, 1.15) 0s"
				}}
			>
				{getExpandedTreeArr(constants.DRAG_FROM.MAIN, 'incomplete').map((taskId, index) => (
					<ListItem
						config={{
			...
			>
				{
					<>
						...
						<div className={classNames("dnd_list-container--drop-area", {
							isDragging: getDragState().isDragging && getDragState().dragItem.dragFrom !== 'completed'
						})}>

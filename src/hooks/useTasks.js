...
const useTasks = () => {
  ...
  function deleteTaskById(taskId, columnId) {
    ...
    const newTaskData = produce(taskData, draftTaskData => {
      ...
      draftTaskData.columns[columnId].taskIds = removeItemByIdInArray(draftTaskData.columns[columnId].taskIds, taskId);
  ...
  function deleteColumn(columnId) {
    const newTaskData = produce(taskData, draftTaskData => {
      ...
      // Removing Item from the array by an id
      draftTaskData.columnOrder = removeItemByIdInArray(draftTaskData.columnOrder, columnId);
    });
    setTaskData(newTaskData);
  }

  return {createTask, createColumn, deleteTaskById, deleteColumn, setTaskData, taskData};
};
